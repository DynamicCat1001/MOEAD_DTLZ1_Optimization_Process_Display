...
char filename[1024];
	...
	sprintf_s(filename,"../../SETTINGS/algorithms/MOEAD-DE.txt", strTestInstance);
...
void CALG_EMO_MOEAD_DE::InitializePopulation()
{
	...
	char filename1[1024];
	sprintf_s(filename1,"../../SETTINGS/weightvectors/W%dD_%d.dat", NumberOfObjectives, s_PopulationSize);		
...
{
	char filename[1024];
	sprintf_s(filename,"../../SAVING/MOEAD-DE/POF/POF_%s_RUN%d.dat",strTestInstance, run_id);
	SaveObjSpace(filename); 

	//sprintf_s(filename,"../Saving/MOEAD-DE/POS_%s_RUN%d.dat",strTestInstance, run_id);
